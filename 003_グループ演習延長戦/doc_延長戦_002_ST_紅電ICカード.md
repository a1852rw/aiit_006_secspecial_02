...
##### ルート権限の禁止(OT.Access)
TOEはアクセス時に「カード利用者」「カード発行専用端末」の役割を認識し、指定した役割のみにパスワードの管理を制限する。役割にTOEの管理者権限を持つものはない。
この動作は「FMT_MTD.1」「FMT_SMF.1」「FMT_SMR.1」を満足する。

##### データ保存への暗号標準の採用(OT.Encryption)
TOE内に保存されるデータは鍵長が2,048BitのRSAアルゴリズムにより暗号化される。
この動作は「FCS_COP.1」を満足する。

##### 通信への暗号標準の採用(OT.Communication)
TOEは、「ICカード読取機」「ICカード発行専用端末」と通信を行う際は鍵長が2,048BitのRSAアルゴリズムにより暗号化されるとともに改竄を検知する。
この動作は「FCS_COP.1」「FDP_ITC.1」を満足する。

##### 物理的耐タンパ機能の搭載(OT.Disassembly)
TOEを分解しようとした場合、内部の回路が破壊されるよう設置されている。回路が破壊されることで保存されているデータは取り出すことができなくなる。
この仕様は「FPT_PHP.3」を満足する。

##### サイドチャネル攻撃に対する耐タンパ機能の搭載(OT.Sidechanel)
TOEには電磁波のノイズ源が埋め込まれている。回路内の消費電力や処理時間が処理の内容によって変化しないよう統計的に出力が処理される機能が搭載されている。
この仕様は「FPT_PHP.3」を満足する。
...
正規のアクセスに対しては利用者、管理者それぞれにアクセス権が割り当てられその範囲内でのみ紅電ICカード内の個人情報にアクセスすることができる。  
